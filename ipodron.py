...
# Погода 1 - ливень ,ураган ... 5 - безоблачный день
weather = randint(1, 5)
...
# Сброс игры
def setupHorse():
    global state02, state01, state03, state04
    ...
    global fastSpeed04, fastSpeed03, fastSpeed02, fastSpeed01
...
def winRound(horse):
    ...
    if horse > 0:
        ...
        if win > 0:
            res += "Поздравляем! Средства уже зачисленны на Ваш счет!"
            insertText(f"Этот забег принес вам {int(win)} {valuta}")
        else:
            res += "К сожалению, ваша лошадь была неправильной. Попробуйте ещё раз!"
            insertText("Делайте новую ставку!")
        messagebox.showinfo("Результат", res)
    else:
        messagebox.showinfo("Все плохо! До финиша никто не дошёл. Забег признан несостоявшимся. Ставки возвращенны.")
    ...
    if money < 1:
        messagebox.showinfo("Стоп без денег все")
        quit(0)
...
def problemHorse():
    ...
    if horse == 1 and play01 == True and x01 > 0:
        if randint(0, maxRand) < state01 * 5:
            ...
            play01 = False
            messagebox.showinfo("Никогда такого небыло, и вот опять", f"Лошадь {nameHorse01} заржала и скинула жокея!")
        elif randint(0, maxRand) < state01 * 5 and not fastSpeed01:
            messagebox.showinfo("Ыеликолепно!", f"{nameHorse01} перестала притворяться и ускорилась!")
            fastSpeed01 = True
    ...
    if horse == 2 and play02 == True and x02 > 0:
        if randint(0, maxRand) < state02 * 5:
            ...
            play02 = False
            messagebox.showinfo("Никогда такого небыло, и вот опять", f"Лошадь {nameHorse02} заржала и скинула жокея!")
        elif randint(0, maxRand) < state02 * 5 and not fastSpeed02:
            messagebox.showinfo("Ыеликолепно!", f"{nameHorse02} перестала притворяться и ускорилась!")
            fastSpeed02 = True
    ...
    if horse == 3 and play03 == True and x03 > 0:
        if randint(0, maxRand) < state03 * 5:
            ...
            play03 = False
            messagebox.showinfo("Никогда такого небыло, и вот опять", f"Лошадь {nameHorse03} заржала и скинула жокея!")
        elif randint(0, maxRand) < state03 * 5 and not fastSpeed03:
             messagebox.showinfo("Dеликолепно!", f"{nameHorse03} перестала притворяться и ускорилась!")
             fastSpeed03 = True
    ...
    if horse == 4 and play04 == True and x04 > 0:
        if randint(0, maxRand) < state04 * 5:
            ...
            play04 = False
            messagebox.showinfo("Никогда такого небыло, и вот опять", f"Лошадь {nameHorse04} заржала и скинула жокея!")
        elif randint(0, maxRand) < state04 * 5 and not fastSpeed04:
            messagebox.showinfo("Ыеликолепно!", f"{nameHorse04} перестала притворяться и ускорилась!")
            fastSpeed04 = True


# Добавления текста в чат
def insertText(s):
...
def getHealth(name, state, win):
    s = f"Лошадь {name} "
    if state == 5:
        s += "мучается несварениемм желудка."
    elif state == 4:
        s += "плохо спала. Подергивается веко."
    elif state == 3:
        s += "сурова и беспощадна."
    elif state == 2:
        s += "в отличном настроение, покушала хорошо."
    elif state == 1:
        s += "просто ракета!"
    s += f"({win}:1)"
    return s
...
def moveHorse():
    ...
    allPlay = play01 or play04 or play03 or play02
...
def refreshCombo(eventObject):
    ...
    lableALLMoney ["text"] = f"У вас на счету: {int(money - summ)} {valuta}."
...
nameHorse01 = "Гнида"
nameHorse02 = "Жопная дырка"
nameHorse03 = "Рыбий пупок"
nameHorse04 = "Чилипиздрик"
...
if (money <= 0):
    ...
    quit(0)
lableALLMoney = Label(text=f"Осталось средств:{money}", font="Arial 12")
lableALLMoney.place(x=20, y=565)

# Чекбоксы и ставки
lableHorse01 = Label(text="Ставка на лошадь №1")
lableHorse01.place(x=20, y=450)

lableHorse02 = Label(text="Ставка на лошадь №2")
lableHorse02.place(x=20, y=480)

lableHorse03 = Label(text="Ставка на лошадь №3")
lableHorse03.place(x=20, y=510)

lableHorse04 = Label(text="Ставка на лошадь №4")
lableHorse04.place(x=20, y=540)
...
# состояние лошадей 1 - великолепна ; 5 - больна
state01 = randint(1, 5)
state02 = randint(1, 5)
state03 = randint(1, 5)
state04 = randint(1, 5)
